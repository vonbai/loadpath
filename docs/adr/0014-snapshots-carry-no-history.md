...
A snapshot is a record rather than a lead, so it carries one schema number and the producer version. Its closed shape is exactly `schema`, `version`, `files`, `dirs` and `spans`; it carries no commit, cutoff or other Git-derived field. `--compare` validates every nested field before rendering and refuses unknown, older or partial shapes rather than reading absent fields as zeros. There is no compatibility reader: a shape this version did not write is not a shape it can diff.
