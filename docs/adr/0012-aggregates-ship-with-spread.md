...
So an average alone is not reportable. Each aggregate carries the min–max across the buckets it summarises, the bucket count as a stated parameter, and a flag when mass concentrates in a single bucket — because that concentration is the signal and the mean over it is not. On identical data, 50 bins produced systematically higher miscalibration than 15, and equal-width bins higher than adaptive ones, so an undisclosed bucketing choice is a hidden assumption.
