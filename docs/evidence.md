...
- **No source supplies a threshold, a precision figure, or a window length.** Every constant in `scan.mjs` — 20 files, 800 lines, 4 windows, a breadth cap of 30, a 5% author share, a 90-day activity horizon — is tunable convention, not a principled value. The horizon is bounded by the window rather than asserted over it: a window shorter than ninety days cannot be asked a ninety-day question, so the line prints the number of days it actually reached back.
