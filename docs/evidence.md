...
- **Every retained result is per-file or per-commit.** Nothing licenses aggregating them to a Source-containing directory, which is the unit this tool emits. Those rollups inherit no authority from this evidence.
- **No retained source gives Source-path depth a decision value, a healthy distribution or a maximum.** The directory-depth distribution explored in the research record produced no placement decision that full paths and Subject ownership did not already answer. The maximum remains orientation only; recursive parent rollup is deferred until a pinned deep-tree fixture demonstrates lost decision context.
- **No source supplies a threshold, a precision figure, or a window length.** Every constant that shapes a measurement here is tunable convention rather than a principled value. This is the register of them, and each is listed with the file that spends it, because a constant whose home nobody can name is a constant nobody tunes. The numbers are not restated in another document.
  ...
  **`report.mjs`, where the page is cut to size.** The L0 caps are 5 languages, 2 test conventions, 5 largest Source-containing directories, 8 relocations, 3 entangled groups and 3 fan-out rows, and a node reaches that last list at 3 or more outward edges with 1 or fewer inward. A comparison names 5 members of a group that formed or dissolved, and reports a Source-containing directory whose mass moved by more than a fifth. The commit-share column is dropped where every Source-containing directory's top author holds every one of its commits. Every one of these states what it dropped; the rule is in `DESIGN.md`.
...
**A shared root dilutes the coverage gate.** A Span is believed only if its analyzer covered a fifth or more of the Source-containing directories in the scope it was pointed at. madge is pointed at a source directory and a src-layout Python package at its own package root, so those denominators are theirs alone. `go list` and the `.csproj` reader are pointed at the ancestor of their own manifests, and where two ecosystems declare themselves at one repository root that ancestor is the root — so the denominator becomes every Source-containing directory in the tree, the other ecosystem's included. A Go module sharing a root with a large TypeScript tree is then judged on a denominator most of which no Go analyzer was ever going to reach, and a correct graph can be refused for it. Split trees measure cleanly, since each Span is rooted at the ancestor of its own manifests: `backend/go.mod` beside `frontend/package.json` gives each of them its own subtree to be judged on. Closing it means counting the directories that could belong to the ecosystem rather than every Source-containing directory in scope, and it is unmeasured here because no corpus of that shape is pinned.
