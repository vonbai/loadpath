...
- `docs/adr/` — the fifteen recorded decisions and what each one traded away. A decision is not re-litigated here.
...
This skill is **auxiliary to that work**: it keeps Subject ownership, source-file placement and file-level facts, Source-containing directory layout, and dependency relationships legible while the agent builds, so the codebase stays maintainable and migratable. It is not a report generator, not a knowledge map, and not an architecture authority.
...
**Single user journey.** One way in and one way through: *authority → orient → focus → read → decide → verify*. Read the project's vocabulary, architecture and Load-bearing paths; run the default view; ask for structure or one subtree only where a Lead earns the detail; read the named code; decide from project authority and structural consequences; then compare a move against its Snapshot and the project's own gates. The flags are progressive depth in this journey, not competing modes or second entry points.
...
- **Release evidence is observable behaviour, not campaign volume.** `npm test` covers acceptance, the shipped Skill contract and reproducible measurements. A small, opt-in mutation sample checks a few highest-consequence anchors after risky changes; it is diagnostic evidence, never a release precondition or a claim of exhaustive suite strength.
...
It does not parse declarations, symbols, interfaces or control flow inside a file. “File-level” here means source placement, extension, size, test convention and last touch. Internal file structure becomes a Finding only after the agent reads it; adding a generic AST axis would widen the product past its placement and load-path purpose.
...
**Report** — receives acquired facts and renders a view at a requested depth. Hides exact view derivations such as distributions, ordering, traversal summaries, alignment, truncation and the token budget; that behaviour behind one supplied-data interface is what makes it deep. It never opens the filesystem, invokes git or calls an analyzer. No acquisition module prints — including the quarantine, which returns a reason as data and never the sentence a reader sees. `tests/loadpath.test.mjs` holds both directions: `deps.mjs` may not lay out output, and `report.mjs` may not import filesystem or subprocess access.

### The seams

**Acquire ↔ render.** Inventory, History and Dependencies acquire facts from external sources and never print. Report derives a self-contained view only from supplied facts and never reacquires them. “Nothing computes and prints” was too broad: a deep renderer must compute exact presentation aggregates and traversal summaries or leak that work through a shallow interface. v0.1.0's real violation was source access mixed with presentation, which is why its truncation was silent and one claim came to live in four files.
...
**Filesystem ↔ git.** Two independent sources, neither silently substituting for the other. Every module speaks repo-root-relative paths; that is an invariant of the common layer. v0.1.0 left this seam unowned, so its two projections used different path namespaces and its own instruction to read them side by side could not be followed.

They must also **admit the same Source population**, and **a Lead must name a current file the reader can open**. Both were broken after the namespaces were fixed: History admitted paths Inventory had refused, including a generated file recognisable only from its header, and retained files since deleted. Inventory is therefore the population authority. Git first applies the shared path predicate, then joins each vote to Inventory's exact current-file set; downstream History computations need no second Source-containing directory filter or compatibility branch. Relocations are the deliberate exception below.

**A record is not a lead, and neither rule reaches it.** Relocations report what this repository has already moved: the side it moved away from is *supposed* to be gone, and what moved is what moved, whatever its file type. Filtering that table to the source population deleted a 482-file `specs → archive` and every documentation move with it — which is exactly the evidence the section exists to carry, since the migration already done is the best evidence of the one in progress. A lead takes the current-file population; a record does not, and states the difference in its own header, because a file count that means something wider there than everywhere else on the page has to read as wider. That the record exists at all is `docs/adr/0009`: the rename records arrive in the history pass already being run, and a skill about structure and migration that cannot say what a repository has already moved is missing the most on-thesis history fact available.
...
|---|---|---|---|
| **Inventory** | robust Source-containing directory distribution plus a size ranking read against it, Source-path depth, and the name tokens spread across the most rows, O(F log F) | flat Source-containing directory table ordered by mass, binary-searched to a token budget, O(V log V) | file-level placement facts for one named subtree, O(F_subtree) |
| **History** | age, commit count, active/dormant Source-containing directory split, O(C) | weighted co-change with breadth cap and time windows, O(Σ min(k,cap)²) | each file's last commit inside the window, for one named subtree, O(Σ paths) |
| **Dependency** | per declared ecosystem: which analyzer ran, Layer depth, entangled group count — or a named absence, O(V+E) | entangled groups with their anchors, and each analyzer unit's layer and group joined where it matches a row, O(V+E) | the load crossing one named subtree's line, counted per counterpart unit, O(V+E); still no edge list, which would contradict the rule above it |

**L0 is the norms and the largest. L1 is the structure. L2 is the detail.** Inventory's L0 summarises a Source-containing directory distribution and then ranks by size, each row printed as a ratio against that distribution. It does not rank by deviation and must not claim to: `files / median` is monotone in `files`, so dividing every row by one constant cannot reorder them, and a heading promising the rows furthest from this repository's norms promised an arithmetic nothing performed. A real deviation ranking — over a distribution the ratio does not already carry — is the clearest unclaimed improvement in the tool, together with the same treatment for History's and Dependency's L0, which report totals.

### Why the distribution comes first

`cmd/cotx 56f` is uninterpretable alone. `files per source-containing directory: median 7, p90 32, max 136` makes every later row readable, and turns `136f` into *19× the median* — which is a fact about the distribution, checkable and exact, not a verdict. One line buys the meaning of every line after it.

L0 is around twenty lines and carries the Source population's file and line totals, maximum Source-path depth, language mix, test ratio, both distributions, the name tokens recurring across the most Source-containing directories, repository age, the active-versus-dormant split, the modules declared by manifests, and the five largest Source-containing directories, each against that median and each carrying the top author's share of its commits beside the raw count. Source-path depth is physical orientation only; it is distinct from dependency Layer depth and from the Deep module relationship, and no numeric maximum decides placement — `docs/adr/0015`. The share is there because it ranked first among sixty-five metrics where the raw count did not, and it is printed *beside* the count rather than instead of it because it is undefined exactly where activity is absent — `docs/adr/0011`. Where one author holds every Source-containing directory's commits the share is dropped and the page says so: a figure that reads the same on every row is width every row pays for and no row uses. The scattered-name tokens are the one Lead here that no retained paper evaluates as a predictor of anything, which `docs/evidence.md` states plainly, so the line is worded as the placement question it is and not as risk. L0 also answers which analyzer to run, because the manifest names the ecosystem.

### Why the structure table must meet a budget

A flat Source-containing directory table does not scale: on a 3,154-row repository it runs to about 14,000 tokens, the same failure as the per-file table it replaced.

What ships is a flat list ordered by mass, truncated by **binary search over how many rows survive** until the rendering fits the budget — Aider's repo map approach, recorded in `docs/research/findings.md`, and a departure from the recursive tree rollup this document once specified. That rollup was specified here and never built: it gives each subtree an allowance and collapses the rest into a per-parent `+N more`, which reads better on some deep trees. It is revisited only when a pinned deep-tree fixture demonstrates that the budgeted, full-path table loses parent context required for a real placement decision; maximum Source-path depth alone is not evidence. The shipped table scales with the budget rather than with the repository, and states once how many Source-containing directories it did not show.
...
Full paths preserve each row's ancestry without indentation's silent wrong-parent failure. Mass order deliberately does not reconstruct the tree: it keeps the highest-information rows under the budget. A recursive parent view earns its interface only under the pinned-fixture criterion above. For placement, every added path segment must pay rent by naming a Subject, expressing an enforced language or project seam, or preserving a Load-bearing path constraint; redundant adjacent taxonomy collapses. This is a semantic rule, not a maximum Source-path depth.
...
Because the skill says *move*, it also has to say whether the move landed, which is what `--snapshot` and `--compare` are for. A Snapshot records what a move changes the moment it lands — the Source population and Source-containing directory layout, and each measured Span's edges, nodes, Layer depth and group membership — and deliberately nothing derived from git. History is append-only: a migration's own commits touch the old home and the new one together, which is exactly the shape co-change reads as Affinity. So comparison answers the axes that move immediately and names the axis it did not compare. `docs/adr/0014` records the split. Only measured Spans enter the record; schema and producer version carry provenance, and `--compare` accepts only that exact closed shape. Unknown, older or partial records are refused at the interface rather than interpreted through compatibility defaults.

"Reasonable" then has two sources, and neither is invented. The first is **the repository's own norms** — whether 136 files in a Source-containing directory is a problem is judged against this repository's median of 7, not against a threshold this skill made up. The second is **a structural invariant stated with its cost**: not "you should split this", but "this directory is transitively reachable from 80 others, so a change to any one of its subjects reaches all 80; splitting by subject lowers that number, and costs N moved files and one broken import path."
