...
They must also **admit the same population**, and **a lead must name a current file the reader can open**. Both were broken after the namespaces were fixed: history admitted paths the walk had refused, including a generated file recognisable only from its header, and retained files since deleted. The filesystem inventory is therefore the population authority. Git first applies the shared path predicate, then joins each vote to inventory's exact current-file set; downstream history computations need no second directory filter or compatibility branch. Relocations are the deliberate exception below.
