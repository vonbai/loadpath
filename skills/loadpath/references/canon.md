...
Four arXiv papers, read in isolation, behind the co-change scoring in `scripts/scan.py`. They agree on more than they disagree, and what they rule out is as useful as what they support.
...
**What no source supplies.** None validates that directory-level co-change tracks erosion at all; the practitioner study explicitly declines the causal claim. None supplies a threshold, a precision figure, or a window length — so every constant in `scan.py` is tunable convention, not a principled value, and should be read that way.
